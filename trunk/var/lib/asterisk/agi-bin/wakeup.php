...
<?php
{
    ...
    $parm_wakeupcallerid = '"Voiper WakeUp" <*94>';
    ...
    if ( !$rc[result] )
        $rc = execute_agi( "STREAM FILE benvenuto_sveglia \"\" ");
		sleep(1);
    ...
    if ( $outc )
    {
        ...
        while ( $out[$i] )
        {
			...
			$rc = execute_agi( "STREAM FILE sveglia_impostata \"\" ");            
            say_wakeup( $wtime );
            $i++;
    ...
    if ( $outc )
    {
        ...
        {
            if ( !$rc[result] )
                $rc = execute_agi( "STREAM FILE sveglia_telefonica \"12\" ");
            if ( !$rc[result] )
                $rc = execute_agi( "STREAM FILE premi_uno \"12\" ");
            if ( !$rc[result] )
                $rc = execute_agi( "STREAM FILE cancella_sveglia \"12\" ");
            if ( !$rc[result] )
                $rc = execute_agi( "STREAM FILE premi_due \"12\" ");
            if ( !$rc[result] )
            {
            ...
            if ( $rc[result] != -1 )
            {
                ...
                else
                {
                    ...
                    $rc[result] = 0;
                    $rc = execute_agi( "STREAM FILE opzione_non_corretta \"\" ");
    ...
    case '49':	// Pressed 1  - This is to schedule a NEW wakeup call
        {
            ...
            while ( !$rc[result] )
            {
                $rc = execute_agi( "GET DATA digita_ora_della_sveglia 15000 4");
...
//                    $rc = execute_agi( "STREAM FILE your \"0123456789\" ");
                ...
                if ( $rc[result] != -1 )
                {
                    ...
                    if ( $rc[result] > 2359 || strlen( $rc[result]) < 4 || substr($rc[result],2,2) > 59 || $rc[result] < 0)
                    {
                        ...
                        $rc = execute_agi( "STREAM FILE orario_non_corretto \"\" ");
                    }
            ...
            if ( $wtime != -1 && $wtime < 1300 )
            {
                ...
                while ( !$rc[result] )
                {
                    ...
                    if ( !$rc[result] )
                        $rc = execute_agi( "GET DATA premi_am_pm 15000 1");
            ...
            else
            {
                ...
                $rc = execute_agi( "STREAM FILE errore_nel_sistema \"\" ");
                if ( !$rc[result] )
                    $rc = execute_agi( "STREAM FILE arrivederci \"\" ");
                if ( !$rc[result] )
...
//                if ( !$rc[result] )
                    $rc = execute_agi( "STREAM FILE impostata_sul_numero \"\" ");
...
//                    if ( !$rc[result] )
                        $rc = execute_agi( "STREAM FILE sveglia_sul_numero \"\" ");
                ...
                $rc = execute_agi( "STREAM FILE sveglia_rimossa \"\" ");
...
function say_wakeup( $wtime )
{
    ...
    $rc = execute_agi( "STREAM FILE alle_ore \"\" ");
