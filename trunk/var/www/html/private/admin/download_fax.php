...
function deleteCheck(f2) {
	...
	if (confirm("Are you sure to delete this fax?"))
  		return ! cancel;
	else
  		return ! ok;
}
...
$display=2;
$path_to_dir = "/var/spool/asterisk/fax";
$skip = $_REQUEST['skip'];
...
function draw_list($file_array, $path_to_dir, $perpage, $skip, $skipped, $index) 
{
	...
	if ($file_array) {
        ...
        foreach ($file_array AS $key=>$thisfile) {
			...
			print "<font style=\"float:left;margin-left:5px;\" color=\"#000000\">".substr($thisfile,6,2)."/".substr($thisfile,4,2)."/".substr($thisfile,0,4)."&nbsp;&nbsp;".substr($thisfile,9,2).":"."".substr($thisfile,11,2).":"."".substr($thisfile,13,2)."</font>";
			print "<a style=\"margin-right:5px;\" onFocus=\"this.blur()\" href=\"/private/admin/download_fax/". $thisfile ."\" target=\"_blank\">"._("Download")."</a>";
			print "<a style=\"margin-right:5px;\" onFocus=\"this.blur()\" href=\"".$_SERVER['SCRIPT_NAME']."?display=2&mode=file&skip=$skip&del=".$thisfile."\" onClick=\"return deleteCheck(this);\">"._("Delete")."</a>";
			print "</div>";
			print "</td></tr>";
		}
	} else {
			print "<tr><td><br><br><center><b>Fax folder is Empty.</b></center></td></tr>";
	}
	...
	if ($skip) {
	    ...
	    $prevtag_pre= "<tr><td><a onFocus=\"this.blur()\" href='?display=2&mode=file&skip=$prevskip'>Previous Page</a>";
	    print "$prevtag_pre";
	    }
    	if (isset($shownext)) {
    	    $nextskip= $skip + $index;
...
<h3><?php echo _("Download Fax TIFF Files:")?></h3>
