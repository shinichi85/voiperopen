        	...
        	if (($file!=".") && ($file!="..") && ($file!="")) {
                ...
                if (array_search('.voiperpn-'.$serialpn,$restore_files) && array_search('.voiperver-'.$voiperversion,$restore_files)) {
                    ...
                    if (array_search('cdr.tar.gz',$restore_files)){
                        ...
                        echo "<li><a class=\"info\" href=\"javascript:decision('Are you sure you want to delete this File Set?','config.php?mode=tools&display=$display&action=deletefileset&dir=$dir&file=$file')\">";
                        echo _("Delete File Set"); echo "<span>"; echo _("Delete this backup set."); echo "</span></a><br></li>";
                        echo "<h5>"; echo _("You have an Error: "); echo "</h5><h7>"; echo _("This version of Backup is Not compatible with the current version of Voiper."); echo "<br>"; echo _("Files restore works only with the same Software version and Pbx Model (partNumber)."); echo "<br><br>"; echo _("For further information please read the Voiper manual."); echo "</h7>";
                }

        } else {
                                echo "<h3>"; echo _("ERROR: its not a BACKUP SET file.");echo "</h3>";
	}
}
