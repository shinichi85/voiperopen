 ...
 */
    ...
    if($_GET['file'])  // conf file requested via menu link
    {
        ...
        if($result) // file read successfully
        {
            ...
            {
                $menuList[$record[0]] = "$PHP_SELF?file=" . $_GET['file'] . "&section=$record[0]";
            }

            if ($_GET['section']) // sidemenu link was clicked (edit section)
            {
                // create HTML header and left menu
                $page->OC_HTML_doHtmlHeader();
                $page->OC_HTML_doSideMenu($menuList);

                // obtain section entries and comments
                $theItems = $conf->OC_getConfSectionItems($_GET['section']);
                if (! $theItems)
                {
                ...
                $page->OC_HTML_doConfigurationForm($_GET['file'],$_GET['section'],$theItems,$conf->_OC_md5);
    ...
    elseif($_GET['reset']) // top menu "re-read conf" pushed
    {
        // create HTML header and left menu
        $page->OC_HTML_doHtmlHeader();
        $page->OC_HTML_doSideMenu();
    ...
    else // default page
    {
        ...
        if($files)
        {
            ...
            {
                $menuList[$file] = "$PHP_SELF?file=$file&section=$file";
                // HTML output
