...
//------------------------------------------------------------------
  ...
  function create(&$conn, &$valueObject) {
    ...
    $telnr = $valueObject->getTelnr();
    $permission = $valueObject->getPermission();
	if (!is_numeric($description)) {
		$description = "'" . mysql_real_escape_string($description) . "'";
	}
    $sql = "INSERT INTO speednr
              (speednr,name,telnr,permission) VALUES
              ('$number',$description,'$telnr','$permission');";
...
//      if ($phonebook_id==0) $phonebook_id = $searchentry[0]->getID();
  ...
  function read(&$conn, $phonebook_id) {
    ...
    $one_entry->setTelnr($row[3]);
    $one_entry->setPermission($row[4]);
...
//      $arr_obj[] = $two_entry;
  ...
  function delete(&$conn, &$valueObject) {
    ...
    if ($phonebook_id > 0) {
        if ($full) { $full = false; }
        $sql = $sql."AND id=$phonebook_id ";
    }

    if ($number != "") {
        if ($full) { $full = false; }
        $sql = $sql."AND speednr LIKE '$number' ";
    }

    if ($description != "") {
        if ($full) { $full = false; }
        $sql = $sql."AND name LIKE '$description' ";
    }

    if ($telnr != "") {
        if ($full) { $full = false; }
        $sql = $sql."AND telnr LIKE '$telnr' ";
    }

    if ($permission != "") {
        if ($full) { $full = false; }
        $sql = $sql."AND permission LIKE '$permission' ";
    }
  ...
  function countAll(&$conn) {
    $sql = "SELECT count(id) FROM speednr";
    $allRows = 0;
    $result = $conn->makequery($sql);
...
class SpeedpagerDAO extends SpeedEntryDAO {
